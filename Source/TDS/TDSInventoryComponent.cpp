...
FAdditionalWeaponInfo UTDSInventoryComponent::GetAdditionalInfoWeapon(int32 IndexWeapon)
{
	...
	if (WeaponSlots.IsValidIndex(IndexWeapon))
	{
		...
		if (!bIsFind)
		{
			UE_LOG(LogTemp, Warning, TEXT("UTDSInventoryComponent::SetAdditioanlInfoWeapon - No Found Weapon with index -%d"), IndexWeapon);
		}
	}
	else
		{
...
int32 UTDSInventoryComponent::GetWeaponIndexSlotgByname(FName IdWeaponName)
{
	int32 result = -1;
	int8 i = 0;
	bool bIsFind = false;
...
bool UTDSInventoryComponent::CheckAmmoForWeapon(EWeaponType TypeWeapon, int8& AviableAmmoForWeapon)
{
	...
	{
		if (AmmoSlot.WeaponType == TypeWeapon)
		{
			AviableAmmoForWeapon = FMath::Max(AmmoSlot.Cout, 0);
