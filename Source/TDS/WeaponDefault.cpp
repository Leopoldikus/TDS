...
void AWeaponDefault::Fire()
{
	...
	if (WeaponSetting.AnimWeaponInfo.AnimWeponFire && SkeletalMeshWeapon
		&& SkeletalMeshWeapon->GetAnimInstance())
	{
		SkeletalMeshWeapon->GetAnimInstance()->Montage_Play(WeaponSetting.AnimWeaponInfo.AnimWeponFire);
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("Before Fire: Round = %d"), AdditionalWeaponInfo.Round);
	AdditionalWeaponInfo.Round = AdditionalWeaponInfo.Round - 1;
	UE_LOG(LogTemp, Warning, TEXT("After Fire: Round = %d"), AdditionalWeaponInfo.Round);
	...
	if (ATDSCharacter* TDSChar = Cast<ATDSCharacter>(GetOwner()))
	{
		if (UTDSInventoryComponent* MyInv = TDSChar->FindComponentByClass<UTDSInventoryComponent>())
		{
			MyInv->SetAdditionalInfoWeapon(TDSChar->CurrentIndexWeapon, AdditionalWeaponInfo);
			UE_LOG(LogTemp, Warning, TEXT("Inventory Updated with Round = %d"), AdditionalWeaponInfo.Round);
		}
	}
}
	...
	else
	{
		...
		if (ShowDebug)
		{
			DrawDebugCone(GetWorld(), ShootLocation->GetComponentLocation(), ShootLocation->GetForwardVector(),
				WeaponSetting.DistanceTrace, GetCurrentDispersion() * PI / 180.f, GetCurrentDispersion() * PI / 180.f, 32,
				FColor::Emerald, false, 0.1f, (uint8)'\000', 1.0f);
		}
	}
	if (ShowDebug)
	{
		...
		ShootLocation->GetComponentLocation() + ShootLocation-> GetForwardVector() * 500.0f, FColor::Cyan, false, 5.f, (uint8)'\000', 0.5f);
		...
		//DrawDebugSpere(GetWorld(), ShootLocation->GetComponentLocation() + ShootLocation->GetForwardVector() * SizeVectorToChangeShootDirectionLogic, 10.f, 8, FColor::Red, false, 4.0f);
	}
	return EndLocation;
}
...
void AWeaponDefault::FinishReload()
{
	...
	int8 NeedToReload = WeaponSetting.MaxRound - AdditionalWeaponInfo.Round;
	int8 AmmoTaken = FMath::Min(NeedToReload, AviableAmmoFromInventory);
	AdditionalWeaponInfo.Round += AmmoTaken;
	WeaponReloading = false;

	AActor* OwnerActor = GetOwner();
	if (UTDSInventoryComponent* MyInv = OwnerActor ? OwnerActor->FindComponentByClass<UTDSInventoryComponent>() : nullptr)
	{
		MyInv->AmmoSlotChangeValue(WeaponSetting.WeaponType, -AmmoTaken);
	}
...
int8 AWeaponDefault::GetAviableAmmoForReload()
{
	int8 AviableAmmoForWeapon = 0;
	AActor* OwnerActor = GetOwner();

	if (ensureMsgf(OwnerActor != nullptr, TEXT("[RELOAD] Owner is NULL!")))
	{
		UTDSInventoryComponent* MyInv = OwnerActor->FindComponentByClass<UTDSInventoryComponent>();
		ensureMsgf(MyInv != nullptr, TEXT("[RELOAD] InventoryComponent not found on Owner!"));

		if (MyInv)
		{
			MyInv->CheckAmmoForWeapon(WeaponSetting.WeaponType, AviableAmmoForWeapon);
		}
	}
	return AviableAmmoForWeapon;
}
